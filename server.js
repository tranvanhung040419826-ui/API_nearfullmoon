...
let totalExecute = 0;
let nearMoonServers = new Map(); 

// Tự động nhận diện tên Sea để hiển thị trên dữ liệu JSON
function getSeaName(placeId) {
    const id = String(placeId);
    if (id === "2753915549") return "Near Full Moon Sea 1";
    if (id === "4442272183") return "Near Full Moon Sea 2";
...
app.post('/update-moon', (req, res) => {
    ...
    // Lưu chuẩn cấu trúc cho script Auto Hop
    nearMoonServers.set(jobid, {
    ...
    console.log(`✅ [Web] Đã nạp thành công Server Near Full! JobId: ${jobid} | Sea: ${placeId}`);
    res.status(200).send("Cập nhật thành công Server Near Full!");
});

// 2. Cổng dành riêng cho Script Lua lấy dữ liệu về để Auto Hop
app.get('/api', (req, res) => {
    const nearMoonDataArray = Array.from(nearMoonServers.values());
    res.json(nearMoonDataArray);
});

// Cơ chế tự động xóa server khỏi danh sách sau 15 phút (Quét dọn mỗi 1 phút)
setInterval(() => {
    const now = Date.now();
    for (let [jobid, data] of nearMoonServers.entries()) {
        if (now - data.updatedAt > 15 * 60 * 1000) { 
            console.log(`🧹 [Web] Hết thời gian, xóa Server cũ: ${jobid}`);
            nearMoonServers.delete(jobid);
        }
    }
}, 60000); 

// 3. Giao diện hiển thị gốc (All Seas)
app.get('/', (req, res) => {
    const nearMoonDataArray = Array.from(nearMoonServers.values());
    ...
    const finalData = {
        ...
        "by": "tranduykhanh",
        "sea_filter": "Near Full Moon Only",
        "total_near_moon_servers": nearMoonDataArray.length,
        "near_moon_data": nearMoonDataArray
    };
    ...
    <head>
        ...
        <title>Near Moon Server Tracker - Active</title>
        <style>
            body {
...
app.listen(PORT, () => {
    console.log(`🚀 Web Near Full Moon đang chạy tại port ${PORT}`);
});
